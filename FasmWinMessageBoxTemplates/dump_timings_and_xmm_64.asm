...
;                                                                              ;
;                               Debug sample:                                  ; 
;   Dump x86-64 general-purpose registers, x87 registers, XMM registers,       ;
...
; Check CPUID features: TSC, SSE, note check XCR0 not required for this sample
cmp eax,1
...
movapd [rbx + 16*01],xmm0
movapd [rbx + 16*02],xmm0
movapd [rbx + 16*03],xmm0
movapd [rbx + 16*04],xmm0
movapd [rbx + 16*05],xmm0
movapd [rbx + 16*06],xmm0
movapd [rbx + 16*07],xmm0
dec rcx
jnz @b
...
fstp qword [rax + 8*06]
fstp qword [rax + 8*07]
; Write SSE registers to save state buffer  
lea rax,[_SseBuffer]
movupd [rax + 16*00],xmm0
movupd [rax + 16*01],xmm1
movupd [rax + 16*02],xmm2
movupd [rax + 16*03],xmm3
movupd [rax + 16*04],xmm4
movupd [rax + 16*05],xmm5
movupd [rax + 16*06],xmm6
movupd [rax + 16*07],xmm7
movupd [rax + 16*08],xmm8
movupd [rax + 16*09],xmm9
movupd [rax + 16*10],xmm10
movupd [rax + 16*11],xmm11
movupd [rax + 16*12],xmm12
movupd [rax + 16*13],xmm13
movupd [rax + 16*14],xmm14
movupd [rax + 16*15],xmm15
...
stosw                  ; Interval between FPU and SSE registers
add rsi,2              ; Skip "ST" string
; Build text block for SSE registers
mov ecx,16             ; ECX = Number of SSE registers ( 16 at 64-bit mode )
.dumpSSE:
mov ax,[rsi]
stosw                  ; Write "XM"
mov al,[rsi+2]
...
mov eax,' =  '
stosd
mov ebp,2              ; EBP = Number of double precision values per one XMM 
.dumpXMM:
mov rax,[rbx]
add rbx,8
push rbx
...
mov ax,0D0Ah
stosw
loop .dumpSSE          ; Cycle for 16 registers
...
_Caption        DB  '  GPR64/FPU/XMM and timings dump',0
...
_SseBuffer      DQ  16*2*8  DUP (?)   ; space for 16 SSE registers
