...
Диалоговая процедура GUI окна диспетчера устройств.
Визуализирует информацию в виде дерева (tree).
...
LRESULT CALLBACK AppViewer(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	static LOGFONT lf;           // Font structure
	...
	case WM_CREATE:
	{
		...
		//hFont = CreateFont(16, 0, 0, 0, FW_DONTCARE, FALSE, TRUE, FALSE, DEFAULT_CHARSET, OUT_OUTLINE_PRECIS,
		//	CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, VARIABLE_PITCH, TEXT("Impact"));

		//hFont = CreateFont(15, 0, 0, 0, FW_DONTCARE, FALSE, TRUE, FALSE, DEFAULT_CHARSET, OUT_OUTLINE_PRECIS,
		...
		hdcScreen = CreateDC("DISPLAY", (PCTSTR)NULL,
		...
		treeDimension = HelperDrawTreeSized(GetTree(), GetBase(), fTab, hdcScreenCompat, hFont);
		fSize = TRUE;
	...
	case WM_PAINT:
	{
		...
		BitBlt(ps.hdc, 0, 0, bmp.bmWidth, bmp.bmHeight, hdcScreenCompat,
	...
	case WM_MOUSEMOVE:
	{
		int mouseX = GET_X_LPARAM(lParam) + xCurrentScroll;
		int mouseY = GET_Y_LPARAM(lParam) + yCurrentScroll;
		HelperOpenCloseMouseLight(hWnd, GetTree(), hdcScreenCompat, mouseX, mouseY,
	...
	case WM_MOUSEWHEEL:
	{
		// int addY = -(short)HIWORD(wParam) / WHEEL_DELTA * 3;
	...
	case WM_KEYDOWN:
	{
		...
		case VK_UP:
			if (fTab)
			{
				...
				BitBlt(hdcScreenCompat, 0, 0, bmp.bmWidth, bmp.bmHeight, NULL, 0, 0, PATCOPY);  // This for blank background
				HelperDrawTreeSized(GetTree(), GetBase(), fTab, hdcScreenCompat, hFont);
				fSize = TRUE;
				InvalidateRect(hWnd, NULL, false);
			}
			else
	...
	int xCurrentScroll, int yCurrentScroll, BOOL& fSize, BOOL forceUpdate)
{
	...
	if (((currentMouse != p->prevMouse) || forceUpdate) && p->openable)
	{
		...
		if (p->opened)
		{
			currentMouse ? index = ID_ITEM_OPENED_LIGHT : index = ID_ITEM_OPENED;
		}
		else
		{
			currentMouse ? index = ID_ITEM_CLOSED_LIGHT : index = ID_ITEM_CLOSED;
		}
		HICON hIcon = GetIconHandleByIndex(index);
...
// This parameters better calculate during draw, because depend on font size,
// current active font settings actual during draw.
RECT HelperDrawNodeLayerSized(PTREENODE pNodeList, int nodeCount, int nodeBaseX, int nodeBaseY,
