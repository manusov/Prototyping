...
mov rdi,rbx
mov ebp,2
.unpackTwoColorFonts:   ; Cycle for unpack 2 color fonts
lea rsi,[rasterFont]
push rdi
mov dh,128
...
jnc .writeColor
cmp ebp,2
mov eax,TEXT_FRONT_COLOR_1
je .writeColor
mov eax,TEXT_FRONT_COLOR_2
.writeColor:
stosd
dec cl
...
pop rdi
add rdi,TEXTURE_WIDTH * 4 * 16 
dec ebp
jnz .unpackTwoColorFonts   ; Cycle for unpack 2 color fonts
...
mov ecx,[r15 - sizeof.APPDATA + APPDATA.shaderProgram]
call [r15 + OGLAPI.ptr_glUseProgram]
;--- Get seconds counter (seconds after application start) --------------------; 
call HelperGetSeconds
;--- Check one second interval for current FPS and MBPS visualization ---------;
lea rcx,[seconds1]
fist dword [rcx + 00]
mov eax,[rcx + 00]
cmp eax,[rcx + 04]
...
DB  '   float ty1 = 0.0f + cory + fs;'              , 0Dh, 0Ah 
DB  '   float ty2 = ky - cory + fs;'                , 0Dh, 0Ah
...
mbpsStamp       dq      ?
mbpsOneBlock    dq      ?
seconds1        dd      ?  ; Used offset fpsStamp1 + 32
seconds2        dd      ?  ; Used offset fpsStamp1 + 36
flagShowFps     dd      ?  ; Used offset fpsStamp1 + 40
flagShowMbps    dd      ?  ; Used offset fpsStamp1 + 44
