...
;       adaptive variant with 64-bit predicates if AVX512BW supported or       ;
;   16-bit predicates if AVX512BW not supported (minimum for AVX512F only),    ; 
...
jnc .errorAvx512
; Set flag for available predicates width, 0 = KMOVW (16), 1 = KMOVQ (64)
bt ebx,30
setc [_K64]
...
; Mode select for adaptive variant:
; use 64-bit predicates if AVX512BW supported, 
; means KMOVQ instruction and hex print 64,
; use 16-bit predicates if AVX512BW not supported (minimum for AVX512F only),
; means KMOVW instruction and hex print 16. 
_K64            DB  ?                 ; 0 means KMOVW, 1 means KMOVQ
