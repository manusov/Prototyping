...
;                                                                              ;
;                               Debug sample:                                  ; 
;   Dump x86-64 general-purpose registers, x87 registers, YMM registers,       ;
...
vmovapd [rbx + 32*01],ymm0
vmovapd [rbx + 32*02],ymm0
vmovapd [rbx + 32*03],ymm0
vmovapd [rbx + 32*04],ymm0
vmovapd [rbx + 32*05],ymm0
vmovapd [rbx + 32*06],ymm0
vmovapd [rbx + 32*07],ymm0
dec rcx
jnz @b
...
lea rax,[_AvxBuffer]
vmovupd [rax + 32*00],ymm0
vmovupd [rax + 32*01],ymm1
vmovupd [rax + 32*02],ymm2
vmovupd [rax + 32*03],ymm3
vmovupd [rax + 32*04],ymm4
vmovupd [rax + 32*05],ymm5
vmovupd [rax + 32*06],ymm6
vmovupd [rax + 32*07],ymm7
vmovupd [rax + 32*08],ymm8
vmovupd [rax + 32*09],ymm9
vmovupd [rax + 32*10],ymm10
vmovupd [rax + 32*11],ymm11
vmovupd [rax + 32*12],ymm12
vmovupd [rax + 32*13],ymm13
vmovupd [rax + 32*14],ymm14
vmovupd [rax + 32*15],ymm15
...
mov ecx,16             ; ECX = Number of AVX registers ( 16 at 64-bit mode )
.dumpAVX:
mov ax,[rsi]
stosw                  ; Write "YM"
mov al,[rsi+2]
...
mov eax,' =  '
stosd
mov ebp,4              ; EBP = Number of double precision values per one YMM 
.dumpYMM:
mov rax,[rbx]
add rbx,8
push rbx
...
mov ax,0D0Ah
stosw
loop .dumpAVX          ; Cycle for 16 registers
...
_Caption        DB  '  GPR64/FPU/YMM and timings dump',0
...
_NoAvxContext   DB  'AVX context not supported by OS.',0
...
_AvxBuffer      DQ  16*4*8  DUP (?)   ; space for 16 AVX registers
