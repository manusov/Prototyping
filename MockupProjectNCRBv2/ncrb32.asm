...
push IDS_STRINGS_POOL              ; Parm#2 = Resource name, used numeric ID
...
jz .stringsPoolFailed              ; Go if pointer = NULL, means error
mov [ebx + APPDATA.lockedStrings],eax  ; Store pointer to strings pool
...
push RT_RCDATA                     ; Parm#3 = Resource type
push IDS_BINDERS_POOL              ; Parm#2 = Resource name, used numeric ID
push [ebx + APPDATA.hResources]    ; Parm#1 = Module handle, this load from DLL 
call [FindResource]                ; Find resource, get handle of block
test eax,eax                       ; EAX = HRSRC, handle of resource block
...
call [LockResource]                ; Lock resource, get address pointer 
test eax,eax                       ; EAX = LPVOID, pointer to resource
jz .controlPoolFailed              ; Go if pointer = NULL, means error
mov [ebx + APPDATA.lockedBinders],eax  ; Store pointer to binders pool
...
push ebx                        ; Parm#1 = Memory block base address  
...
mov ecx,[Registry]
push [ecx + APPDATA.hImageList]  ; Parm#1 = Handle to image list
...
push [ecx + APPDATA.hTab]        ; Parm#1 = Container window handle
...
push [ecx + APPDATA.hResources]   ; Parm#1 = Resource module handle
...
cld
mov edi,[Registry]
mov esi,[edi + APPDATA.lockedBinders]
movzx ecx,ax
...
BindInfo:
mov esi,[edi + REGISTRY32.allocatorTempBuffer.objectStart]
add esi,eax
jmp BindEntry
...
movzx eax,ax
mov esi,[Registry]
mov esi,[esi + APPDATA.lockedStrings]
call IndexString      ; Return ESI = Pointer to selected string
push esi              ; Parm#4 = lParam = Pointer to string
push 0                ; Parm#3 = wParam = Not used
