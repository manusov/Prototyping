...
;                                                                              ;
;                             Customized for                                   ; 
;        Intel AMX (Advanced Matrix Extension) test and registers dump.        ; 
;                                                                              ;
...
;                                                                              ;
;       THIS CODE YET NOT VERIFIED AT REAL HARDWARE WITH INTEL AMX SUPPORT.    ;
;                                                                              ;
;==============================================================================;
...
;---------- Dump configuration register TILECFG from buffer -------------------;  
mov ecx,NAME_COLOR
call SetFgColor              ; Set foreground color by input ECX
lea rcx,[NameAmxConfig]      ; Parm#1 = RCX = Pointer to string for output         
mov rdx,ALIAS_REPORTHANDLE   ; Parm#2 = RDX = Report file handle
mov r8,ALIAS_REPORTNAME      ; Parm#3 = R8  = Report file name
call ConsoleWrite            ; Output first message, output = display + file
push rbx
lea rsi,[CONTEXT_BASE]
mov ebx,CONTEXT_SIZE
call DumpMemory
pop rbx

;---------- Blank buffer with 11h, for differentiate write 0 and no writes ----;
lea rdi,[TMM_BASE]
mov ecx,ALL_TMM_SIZE / 8
mov rax,1111111111111111h
rep stosq

;---------- Save data registers TMM0-TMM7 to buffer ---------------------------;
lea rcx,[TMM0_BASE]                    ; RCX = Base address
mov edx,ONE_TMM_SIZE                   ; RDX = Addend for TMM register size
mov eax,64                             ; RAX = Index = Stride for AMX load/save
DB 0C4h, 0E2h, 07Ah, 04Bh, 004h, 001h  ; tilestored [rcx + rax],tmm0
add rcx,rdx
DB 0C4h, 0E2h, 07Ah, 04Bh, 00Ch, 001h  ; tilestored [rcx + rax],tmm1
add rcx,rdx
DB 0C4h, 0E2h, 07Ah, 04Bh, 014h, 001h  ; tilestored [rcx + rax],tmm2
add rcx,rdx
DB 0C4h, 0E2h, 07Ah, 04Bh, 01Ch, 001h  ; tilestored [rcx + rax],tmm3
add rcx,rdx
DB 0C4h, 0E2h, 07Ah, 04Bh, 024h, 001h  ; tilestored [rcx + rax],tmm4
add rcx,rdx
DB 0C4h, 0E2h, 07Ah, 04Bh, 02Ch, 001h  ; tilestored [rcx + rax],tmm5
add rcx,rdx
DB 0C4h, 0E2h, 07Ah, 04Bh, 034h, 001h  ; tilestored [rcx + rax],tmm6
add rcx,rdx
DB 0C4h, 0E2h, 07Ah, 04Bh, 03Ch, 001h  ; tilestored [rcx + rax],tmm7

;---------- Dump data registers TMM0-TMM7 from buffer -------------------------;
lea rsi,[TMM0_BASE]
xor edi,edi
.tmmDump:
mov ecx,NAME_COLOR
call SetFgColor              ; Set foreground color by input ECX
lea rcx,[NameAmxTmm]         ; Parm#1 = RCX = Pointer to string for output         
mov rdx,ALIAS_REPORTHANDLE   ; Parm#2 = RDX = Report file handle
mov r8,ALIAS_REPORTNAME      ; Parm#3 = R8  = Report file name
call ConsoleWrite            ; Output first message, output = display + file
push rbx rdi
xchg eax,edi
lea rdi,[TEMP_BUFFER + 2048]
mov rcx,rdi
mov bl,0
call DecimalPrint32
mov al,0
...
TitleString        DB  'Hardware Shell v0.21 (x64)', 0
CrLf2              DB  0Dh, 0Ah
CrLf               DB  0Dh, 0Ah, 0
